...
import {toggleCompleatedTodo, removeTodo} from '../features/todo/todoSlice'
const TodoItem = ({todo}) => {

    const dispatch = useDispatch();
    const toggleTodoHandler = (id) => {
        dispatch(toggleCompleatedTodo(id))
    }
    ...
    return (
        <div className='flex justify-between items-center my-2'>
            ...
            <div className={`text-sm ${todo.compleated  ? 'line-through font-medium text-lime-400' : ''}`}>
                {todo.text}
            </div>
